...
class IncomeRequest extends FormRequest
{
    ...
    public function rules()
    {
        $rules = [
            ...
            'tax_year_id' => ['bail', 'required', 'integer', 'exists:tax_years,id'],

            // Assume income file must be of below mime types and under 10MB
            'income_file' => ['nullable', 'bail', 'file', 'mimes:jpg,jpeg,png,webp,doc,docx,xls,xlsx,pdf', 'max:10240']
        ];
        ...
        if (!empty($this->tax_year_id) and TaxYear::where('id', $this->tax_year_id)->exists()) {
            $rules['income_date'] = ['bail', 'required', 'string', 'date_format:Y-m-d', new ValidIncomeDate($this->tax_year_id)];
        } else {
            $rules['income_date'] = ['bail', 'required', 'string', 'date_format:Y-m-d'];
        }
